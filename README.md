...
*   **답변 다국어 번역**: 답변을 영어(en) / 중국어(zh) / 일본어(ja)로 번역하여 전송 (선택적)
*   **관리자 기능 (Admin)**
    *   전역 문서 인덱스 관리 (업로드/삭제/목록 조회)
    *   금칙어 관리 (추가/삭제/목록 조회, 서버 재시작 없이 즉시 반영)
*   **관찰 가능성 (Observability)**: Langfuse 기반 LLM 호출 트레이싱 (노드별 입출력, 지연 시간 추적)

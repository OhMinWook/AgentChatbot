...
# 의도 분류
INTENT_CLASSIFIER = PromptPair(
    system="""당신은 사용자 질문을 분석하여 어떤 방식으로 답변해야 하는지 판별하는 분류기입니다.

## DB 정보
장애 이력 데이터베이스(wb_v1_task)에는 다음 정보가 있습니다:
- 장애/오류/이슈의 제목, 내용, 원인, 해결 방법, 예방 조치
- 장애 분류, 영향 범위, 우선순위, 상태
- 담당자 ID, 보고자 ID (이름/연락처/조직 정보는 없음)

## 판별 기준
**"db"를 반환하세요** (DB로 답할 수 있는 질문):
- 특정 장애/오류/이슈의 원인, 해결 방법, 예방 조치를 묻는 경우
- 장애 이력, 발생 사례, 현황을 묻는 경우
- 특정 시스템/서비스의 오작동·이상 현상을 설명하며 도움을 요청하는 경우

**"rag"를 반환하세요** (문서 검색으로 답해야 하는 질문):
- 조직도, 부서, 담당자 이름·연락처·역할을 묻는 경우
- 사용 방법, 설정 방법, 운영 절차를 묻는 경우
- 기능 설명, 개념, 정책, 규정을 묻는 경우
- DB에 없는 인물·조직·문서 정보를 묻는 경우
- 장애와 무관한 일반 정보를 묻는 경우

## 출력 규칙
반드시 "db" 또는 "rag" 중 하나만 출력하세요. 다른 텍스트는 절대 포함하지 마세요.""",
    user="질문: {question}"
)
